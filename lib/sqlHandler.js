...
/**
 * Helper function to build the base sequelize model for a particular resource,
 * as well as the 
 */
SqlStore.prototype._buildModels = function() {
  var self = this;

  // Since our attributes include relationship definitions, we filter those out here.
  // Build an array of the attributes that are not relationships
  ...
  // Pick the attributes that are not relationships
  localAttributes = _.pick(self.resourceConfig.attributes, localAttributes);

  // Build an array of the attributes that are relationships
  ...
  // Pick the attributes that are relationships
  relations = _.pick(self.resourceConfig.attributes, relations);

  // Use our helper function to initialize our model attributes
  var modelAttributes = self._joiSchemaToSequelizeModel(self.resourceConfig.resource, localAttributes);

  // Build the actual sequelize model.
  // TODO: how often is this called? It better not be every request...
  // We want to be able to specify the database table name separately from the
  // resource name, so we add an optional property to the resourceConfig object.
  self.baseModel = self.sequelize.define(
      self.resourceConfig.table || self.resourceConfig.resource,
      modelAttributes,
      { timestamps: false }
    );

  // Now we build out the models needed to support the way JSON API deals with
  // relationships. In particular, JSON API exposes each relationship as
  // a unique resource, with which you can interact using the normal HTTP verbs.
  // In cases where we have relationships defined in a separate table, we will
  // need separate sequelize models; but in cases where we have a one-to-one
  // relationship defined simply using a foreign key, we'll instead need to
  // handle that by reading and writing to/from the base model.
  // We may still define a sequelize model for those cases, but we would need to
  // override any actual database queries.
  self.relations = { };
  self.relationArray = [ ];
...
/**
 * Helper function to initialize our sequelize model attributes.
 * This lets us add fields like id, type, and meta,
 * as well as perform some type conversions from the Joi definitions.
 */
SqlStore.prototype._joiSchemaToSequelizeModel = function(resourceType, joiSchema) {
  var model = {
...
/**
 * This function defines the model for a relationship resource!
 * (The relationship itself is a resource, and gets its own sequelize model.)
 * This only fits the existing ipdatatel table schema in certain cases;
 * Often, we just have a key in one table linking it directly to another.
 * In those cases this is probably not the appropriate function to call.
 */
SqlStore.prototype._defineRelationModel = function(relationName, many) {
  ...
  // The way this relationship resource 
  var modelProperties = {
    uid: {
...
SqlStore.prototype._generateSearchBlock = function(request) {
  ...
  debug('_generateSearchBlock, searchBlock:', searchBlock);
  return searchBlock;
};
...
SqlStore.prototype.search = function(request, callback) {
  ...
  self.baseModel.count(options).asCallback(function(err, count) {
    ...
    options.attributes = {exclude: ['type', 'meta']};
    debug("options:", options);
    console.log("baseModel:", self.baseModel);
